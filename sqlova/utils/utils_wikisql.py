...
def get_loader_wikisql(data_train, data_dev, bS, shuffle_train=True, shuffle_dev=False):
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers=4,
        collate_fn=my_collate_fn  # now dictionary values are not merged!
    )
...
def get_fields_1(t1, tables, no_hs_t=False, no_sql_t=True):
    nlu1 = t1['question']
...
def get_fields(t1s, tables, no_hs_t=False, no_sql_t=False):
    ...
    for t1 in t1s:
        if no_hs_t:
            nlu1, nlu_t1, tid1, sql_t1, tb1, hs_t1, hs1 = get_fields_1(t1, tables, no_hs_t, no_sql_t)
        else:
            nlu1, nlu_t1, tid1, sql_t1, tb1, hs_t1, hs1 = get_fields_1(t1, tables, no_hs_t, no_sql_t)
    ...
    return nlu, nlu_t, sql_t, tb, hs_t, hs
