...
def test(t, data_table, model, model_bert, bert_config, tokenizer,
    ...
    for i in range(len(pr_sql_i)):
        try:
            pr_ans_i, _ = engine.execute_return_query(tb[i]['id'], pr_sc[i], pr_sa[i], pr_sql_i[i]['conds'])
            print("answer found")
            print(pr_ans_i)
            pr_ans.append(pr_ans_i[0])
...
if __name__ == '__main__':
    ...
    # t=[{'table_id': '1-10015132-11', 'phase': 1, 'question': 'What position does the player who played for butler cc (ks) play?', 'question_tok': ['What', 'position', 'does', 'the', 'player', 'who', 'played', 'for', 'butler', 'cc', '(', 'ks', ')', 'play', '?'], 'sql': {'sel': 3, 'conds': [[5, 0, 'Butler CC (KS)']], 'agg': 0}, 'query': {'sel': 3, 'conds': [[5, 0, 'Butler CC (KS)']], 'agg': 0}, 'wvi_corenlp': [[8, 12]], 'bertindex_knowledge': [0, 4, 0, 0, 4, 0, 0, 0, 1, 2, 2, 2, 3, 0, 0], 'header_knowledge': [1, 0, 0, 1, 0, 2]}]
    #t=[{'table_id': '1-ftable1', 'phase': 1, 'question': 'Which company has more than 100 employees?', 'question_tok': ['Which', 'company', 'has', 'more', 'than', '100', 'employees', '?'], 'bertindex_knowledge': [0, 4, 0, 0, 4, 0, 0, 0, 1, 2, 2, 2, 3, 0, 0], 'header_knowledge': [2,1]}]
    ...
    with torch.no_grad():
        acc_dev, results_dev, cnt_list= test(t,
                                                dev_table,
                                                model,
                                                model_bert,
                                                bert_config,
    ...
    if args.do_infer:
        ...
        # nlu1 = "Tell me what the notes are for South Australia"
        ...
        data_table = load_jsonl('./data_and_model/dev_tok.tables.jsonl')
        # table_name = 'ftable2'
        table_name='table_10015132_11'
