...
class DBEngine:
    ...
    def __init__(self, fdb):
        try:
            ...
            print("Connected to SQLite database using Records.")
    ...
    def show_all_tables(self):
        try:
            query_result = self.db.query("SELECT name FROM sqlite_master WHERE type='table'")
            tables = [table.name for table in query_result]
            print("Tables in the database:")
            for table in tables:
                print(table)
        except Exception as e:
    ...
    def execute_query(self, table_id, query, *args, **kwargs):
        try:
            query_result = self.db.query(query, *args, **kwargs)
    ...
    def execute(self, table_id, select_index, aggregation_index, conditions, lower=True):
        try:
            ...
            agg = "MAX" if aggregation_index == 0 else "MIN" if aggregation_index == 1 else ""
            where_clause = []
            where_values = []
            for col_index, op, val in conditions:
                if lower and isinstance(val, str):
                    val = val.lower()
                where_clause.append(f"col{col_index} {op} :val{col_index}")
                where_values.append(f"val{col_index}")
            where_str = ' AND '.join(where_clause)
            query = f"SELECT {agg}({select}) AS result FROM {table_id} WHERE {where_str}"
            query_result = self.db.query(query, **{f"val{index}": value for index, value in enumerate(where_values, start=1)})
            return query_result.all()
        except Exception as e:
            print(f"Error executing query: {e}")
            return None
    ...
    def execute_return_query(self, table_id, select_index, aggregation_index, conditions, lower=True):
        try:
            ...
            where_str = ' AND '.join(where_clause)
            query = f"SELECT {agg}({select}) AS result FROM {table_id} WHERE {where_str}"
            return self.db.query(query, **{f"val{index}": value for index, value in enumerate(where_values, start=1)}), query
        except Exception as e:
            print(f"Error executing query: {e}")
            return None
    ...
    def show_table(self, table_id):
        try:
            if not table_id.startswith('table'):
                table_id = 'table_{}'.format(table_id.replace('-', '_'))
            rows = self.db.query('SELECT * FROM {}'.format(table_id))
            for row in rows:
                print(row)
        except Exception as e:
